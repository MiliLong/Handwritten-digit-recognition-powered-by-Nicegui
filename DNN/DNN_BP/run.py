...
class TrainDNN(DNN):
    ...
    def train(self):
        ...
        epochs = int(self.sample_config["epochs"])
        ...
        for i in range(epochs):
            for count in tqdm(range(train_count), desc="The {0}th epoch".format(i+1), unit="iteration", bar_format="{l_bar}{bar}{r_bar}", colour='blue'):
                ...
                target = cp.zeros(int(self.dist["output_num"])) + 0.001
    ...
    def test(self):
        ...
        text.append("训练数据总数: {0} ".format(int(self.dist["samples_num"])))
    ...
    def run(self):
        ...
        result = "预测结果: {0}\n预测概率: {1}".format(cp.argmax(output_act), output_probability)
...
def main():
    ...
    if config["ways"] == "test":
        ...
        with open('DNN/DNN_BP/logs.txt', 'w', encoding="utf-8") as t:
            t.write(result)
    elif config["ways"] == "train":
        dnn.train()
        result = dnn.test()
        with open('DNN/DNN_BP/logs.txt', 'w', encoding="utf-8") as t:
            t.write(result)
    elif config["ways"] == "run":
        result = dnn.run()
        with open('DNN/DNN_BP/logs.txt', 'w', encoding="utf-8") as t:
            t.write(result)
    config["ways"] = ""
